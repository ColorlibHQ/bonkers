<?php
$bonkers_enable_section = bonkers_get_option( 'bonkers_addons_video_enable', true );
if ( $bonkers_enable_section || is_customize_preview() ) :
?>
    <div id="bonkers-video-section" class="bonkers-video-section" <?php if( false == $bonkers_enable_section ): echo 'style="display: none;"'; endif; ?>>
    <?php $bonkers_video_title = get_option( 'bonkers_addons_video_title', esc_html__( 'Your success is our most important priority', 'bonkers' ) ); ?>
        ...
        <?php $bonkers_addons_video_url = get_option( 'bonkers_addons_video_url' ); ?>
            <?php echo do_shortcode( '[video src="' . esc_url( $bonkers_addons_video_url ) . '"]' ); ?>
        </div>
