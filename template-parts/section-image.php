<?php
$bonkers_enable_section = bonkers_option( 'image_enable', bonkers_option( 'services_enable', true ) );
if ( $bonkers_enable_section || is_customize_preview() ) :

	$bonkers_image_css_class = 'bonkers-image-left';
	$bonkers_image_layout    = bonkers_option( 'image_layout', 'left' );
	...
	<div id="bonkers-image-section" class="bonkers-image-section <?php echo esc_attr( $bonkers_image_css_class ); ?>" <?php echo $bonkers_enable_section ? '' : 'style="display: none;"'; ?>>
		...
		<?php
		$bonkers_image_image = bonkers_option( 'image_image', esc_url( get_template_directory_uri() ) . '/assets/images/StockSnap_JBW2PXDOL6.jpg' );
		if ( is_numeric( $bonkers_image_image ) ) {
			$bonkers_image_image = wp_get_attachment_url( $bonkers_image_image );
		}
		$bonkers_image_title      = bonkers_option( 'image_title', esc_html__( 'Start Growing your Business', 'bonkers' ) );
		$bonkers_image_link_title = bonkers_option( 'image_link_title', esc_html__( 'Learn More', 'bonkers' ) );
		?>
		...
		<div class="bonkers-image-section-text">
			...
			<div class="bonkers-image-section-content">
				<?php
				$bonkers_image_text = bonkers_option( 'image_text' );
				echo wp_kses_post( $bonkers_image_text );
				?>
				<?php if ( ! empty( $bonkers_image_link_title ) || is_customize_preview() ) { ?>
					<div class="clearfix"></div>
					<a href="<?php echo esc_url( bonkers_option( 'image_link_url', '#' ) ); ?>" class="ql_border_btn"><?php echo esc_html( $bonkers_image_link_title ); ?></a>
				<?php } ?>
			</div>
		</div>
