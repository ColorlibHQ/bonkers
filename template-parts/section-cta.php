<?php
$bonkers_enable_section = bonkers_option( 'cta_enable', true );
if ( $bonkers_enable_section || is_customize_preview() ) :
	$bonkers_cta_image = bonkers_option( 'cta_image', esc_url( get_template_directory_uri() ) . '/assets/images/StockSnap_R7GVMRJWW9.jpg' );
	if ( is_numeric( $bonkers_cta_image ) ) {
		$bonkers_cta_image = wp_get_attachment_url( $bonkers_cta_image );
	}
	$bonkers_cta_link_title = bonkers_option( 'cta_link_title', esc_html__( 'Sign Up', 'bonkers' ) );
	$style                  = '';
	...
	<div id="bonkers-cta-section" class="bonkers-cta-section" <?php echo $style; ?>>
		<h2 class="bonkers-cta-title"><?php echo wp_kses_post( bonkers_option( 'cta_title' ) ); ?></h2>
